function [y_all, y_average, y_ave2, hplot] = display_psydat_v2(s_name, exp_name, pstyle, param_perm) 

% analyse raw data in psydat-file, average them, and plot them
% ----------------------------------------------------------------------
% Usage: [y_all, y_average, y_ave2, hplot] = display_psydat_v2(s_name, exp_name, pstyle, param_perm) 
%
%     Uses read_psydat to extract all data for subject s_name and
%     experiment exp_name from a psydat file  -- WHICH IS IN VERSION 2 FORMAT -- 
%     Then sort and combine data into different subgroups, one for
%     each UNIQUE combination of values of M.PARAM(:).  Calculates the
%     mean across threshold-data of different runs, individually for
%     each data subgroup.
%
...
% extract data from psydat file, in old version 2 format:
[x, y] = read_psydat_v2(s_name, exp_name);
...
% End of file:  display_psydat_v2.m
