% Usage: mpsy_simu_main_mult_reps_template
% ----------------------------------------------------------------------
...
disp('ACHTUNG, veraltet  ?!\n');
disp('ACHTUNG ACHTUNG, es gibt neue Version: \n'');
disp('neue template generierung als mittel ueber n reps geschieht in mpsy_simulation\n');
return
...
switch M.MODE
  ...
  case M_SIMULATION,
    ...
    % generate new signals by use of the "user-script".  Here only
    % for calcuation of the template:
    eval([M.EXPNAME 'user']);
    mpsy_template;
  

    if SIMU.NUM_TEMPLATES > 1,
      m_template_sum = m_template;   % save the already calculated first template
      rep_ref0_sum  = rep_ref0;      % save already calculated rep of the masker alone

      for kk = 1:(SIMU.NUM_TEMPLATES-1),
        eval([M.EXPNAME 'user']);      % produce new signals, again
        mpsy_template;                 % generate new template and rep0, again
